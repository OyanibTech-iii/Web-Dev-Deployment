document.addEventListener('DOMContentLoaded', function () {
    ...
    applyCropBtn?.addEventListener('click', () => {
        ...
        canvas.toBlob((blob) => {
            ...
            // --- NEW: UPLOAD IMAGE IMMEDIATELY AFTER CROPPING ---
            ...
            if (previewLarge) {
                ...
                previewLarge.style.display = 'block'; // Ensure it's visible if it was hidden by onerror
            } else {
                ...
                if (initialsLarge) initialsLarge.style.display = 'none'; // Hide initials
    ...
    function uploadCroppedImage(blob) {
        ...
        loadingModal.id = 'image-upload-loading-modal';
        loadingModal.className = 'fixed inset-0 z-[110] backdrop-blur-md flex items-center justify-center';
        loadingModal.innerHTML = `
            ...
            <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full mx-4 text-center relative z-10">
                ...
                <h3 class="text-lg font-semibold text-gray-800 mb-2">Saving Photo</h3>
                <p class="text-sm text-gray-500">Please wait while we update your profile image...</p>
            </div>`;
        ...
        .then(data => {
            document.getElementById('image-upload-loading-modal')?.remove();
            if (data.success) {
                // We don't necessarily need to reload here because we updated the preview
                // but let's show a quick toast if available
                if (window.growficoToast) {
                    window.growficoToast({ type: 'success', message: 'Profile image saved!' });
                }
            } else {
        ...
        .catch(error => {
            document.getElementById('image-upload-loading-modal')?.remove();
            showResult('Error', 'Failed to upload image', 'error');
        });
    }

    // --- Form Submission (Information Only) ---
    document.getElementById('profile-form-main')?.addEventListener('submit', function (e) {
        ...
        // Remove image from this form if it was accidentally included
