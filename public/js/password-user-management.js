...
function initPasswordManagement() {
    ...
    // Request Change for Google Users
    document.getElementById('request-password-btn')?.addEventListener('click', function () {
        const btn = this;
        const originalHtml = btn.innerHTML;
        
        btn.disabled = true;
        btn.innerHTML = '<ion-icon name="sync-outline" class="animate-spin"></ion-icon> Sending...';

        fetch('/profile/request-password-change', {
            method: 'POST',
            headers: { 'X-Requested-With': 'XMLHttpRequest' },
            credentials: 'same-origin',
        })
        ...
        .then(data => {
            if (data.success) {
                showResult('Request Sent', data.message, 'success');
                btn.innerHTML = '<ion-icon name="checkmark-done-outline"></ion-icon> Request Sent';
                btn.classList.replace('bg-dark-forest-green', 'bg-gray-400');
            } else {
                showResult('Request Failed', data.message, 'error');
                btn.disabled = false;
                btn.innerHTML = originalHtml;
            }
        })
        .catch(() => {
            showResult('Error', 'An unexpected error occurred while sending the request.', 'error');
            btn.disabled = false;
            btn.innerHTML = originalHtml;
        });
    });
